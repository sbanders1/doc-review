...
1. First check if a dev server is already running on port 6173:
   ```bash
   lsof -i :6173 2>/dev/null
   ```
...
2. Launch the dev server in the background using Bash with `run_in_background: true`:
   ```bash
   source ~/.nvm/nvm.sh && npm run dev -- --port 6173
   ```

3. Remind the user they can access the app via SSH port forwarding:
   `ssh -L 6173:localhost:6173 sanderson@<host>`
