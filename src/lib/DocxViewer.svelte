<script>
  ...
  onMount(async () => {
    ...
    try {
      await renderAsync(content, docxContentEl, null, {
        className: 'docx',
        breakPages: true,
        renderHeaders: true,
        renderFooters: true,
      ...
      renderError = 'Failed to render document. The file may be corrupted or in an unsupported format.';
    }
...
<div class="docx-container" bind:this={scrollArea} onclick={handleClick}>
  <div class="page-wrapper" bind:this={pageWrapper}>
    {#if renderError}
      <div class="render-error">{renderError}</div>
    {/if}
    <div class="docx-content" bind:this={docxContentEl}></div>
...
<style>
  ...
  /* docx-preview renders pages as sections with class "docx".
     Style the container to center pages with spacing. */
  .docx-content {
    display: flex;
  ...
  /* Style the rendered page sections */
  .docx-content :global(section.docx) {
    background: white;
  ...
  /* Highlight layer */
  .docx-container :global(.highlight-layer) {
    position: absolute;
    top: 0;
    left: 0;
    right: 0;
  ...
  .docx-container :global(.highlight-rect) {
    position: absolute;
  ...
  .docx-container :global(.highlight-rect:hover) {
    background: rgba(255, 200, 0, 0.35);
    border-bottom-color: rgba(255, 200, 0, 0.9);
  }

  .docx-container :global(.highlight-rect.active) {
    background: rgba(100, 108, 255, 0.25);
    border-bottom: 2px solid rgba(100, 108, 255, 0.8);
  }

  .docx-container :global(.highlight-rect.resolved) {
    opacity: 0.4;
  }
